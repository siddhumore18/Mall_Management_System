...
export const InteractiveProductDemo: React.FC = () => {
  ...
  return (
    <section className="relative z-10 px-4 md:px-8 py-12 max-w-6xl mx-auto space-y-8">
      
      {/* Section Header */}
      <div className="text-center space-y-3">
        <div className="inline-flex items-center gap-2 bg-gradient-to-r from-amber-100 to-yellow-100 border border-amber-300 text-amber-900 px-4 py-1.5 rounded-full text-xs font-black tracking-wide shadow-xs">
          <Sparkles className="w-4 h-4 text-amber-700 animate-spin-slow" />
          <span>INTERACTIVE PRODUCT TOUR & SANDBOX</span>
        </div>
        ...
        {rolesConfig.map(r => {
          ...
          return (
            <button
              ...
              className={`px-3.5 py-2.5 rounded-2xl text-xs font-bold transition-all flex items-center gap-2 shrink-0 cursor-pointer border ${
                isActive
      ...
      <div className="bg-stone-900 rounded-3xl border-2 border-amber-300/80 shadow-[0_20px_50px_-15px_rgba(217,119,6,0.2)] overflow-hidden">
        ...
        <div className="px-4 py-3 bg-stone-950 border-b border-stone-800 flex items-center justify-between text-xs text-stone-400">
          <div className="flex items-center gap-2">
            ...
            <span className="ml-3 hidden sm:inline-block font-mono text-[11px] text-stone-400">MegaMart Enterprise Retail Cloud • Sandbox Simulator</span>
          </div>

          <div className="bg-stone-900 px-4 py-1 rounded-full border border-stone-800 font-mono text-[10px] text-amber-400 flex items-center gap-2 max-w-xs truncate">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
            <span>https://app.megamart.com/{selectedRole.toLowerCase()}</span>
          </div>

          <button
            onClick={() => handleTestDriveRole(selectedRole)}
            className="bg-amber-500 hover:bg-amber-400 text-amber-950 font-black px-3 py-1 rounded-lg text-[10px] transition cursor-pointer flex items-center gap-1 shadow-xs"
          >
        ...
        <div className="bg-[#FAF8F5] p-4 md:p-6 min-h-[460px] text-stone-900">
          
          {/* 1. CASHIER POS SIMULATOR */}
          {selectedRole === 'CASHIER' && (
            <div className="space-y-4">
              {/* POS Top Bar */}
              <div className="flex flex-wrap items-center justify-between gap-3 bg-white p-3.5 rounded-2xl border border-amber-200/80 shadow-xs">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-xl bg-amber-600 text-white flex items-center justify-center font-black text-xs">
                    01
                  </div>
                  <div>
                    <h4 className="font-extrabold text-stone-900 text-xs sm:text-sm">Register #01 — Express Checkout</h4>
                    <p className="text-[10px] text-stone-500">Cashier: Demo Staff • Store: Bandra Flagship Supercenter</p>
                  </div>
                </div>

                {/* Customer CRM Lookup in POS */}
                <div className="flex items-center gap-2 bg-amber-50/80 px-3 py-1.5 rounded-xl border border-amber-200 text-xs">
                  <Users className="w-3.5 h-3.5 text-amber-800" />
                  <div>
                    <span className="font-bold text-amber-950">{customerName}</span>
                    <span className="text-[10px] text-amber-700 ml-1.5">({customerPhone}) • <strong>{loyaltyPoints} Pts</strong></span>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                ...
                <div className="md:col-span-7 space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-stone-700 uppercase tracking-wider">Fast-Moving Grocery Quick Scan</span>
                    <span className="text-[10px] text-amber-800 font-semibold bg-amber-100 px-2 py-0.5 rounded-full">Click item to scan</span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {[
                      ...
                      <button
                        ...
                        className="p-2.5 bg-white hover:bg-amber-50 border border-amber-200/90 rounded-2xl text-left transition hover:border-amber-400 group cursor-pointer shadow-2xs"
                      >
                        <div className="flex justify-between items-start">
                          <span className="text-[9px] font-bold text-amber-800 bg-amber-100/70 px-1.5 py-0.5 rounded">{item.tag}</span>
                          <Plus className="w-3.5 h-3.5 text-amber-700 opacity-0 group-hover:opacity-100 transition" />
                        </div>
                        <h5 className="font-bold text-stone-900 text-xs mt-1.5 truncate">{item.name}</h5>
                  ...
                  <div className="bg-emerald-50 border border-emerald-200 p-3 rounded-2xl text-xs text-emerald-900 flex items-center gap-2">
                    <Zap className="w-4 h-4 text-emerald-700 shrink-0" />
                    <span><strong>Sub-Second POS Speed:</strong> Supports physical USB & Bluetooth barcode scanners with zero keyboard latency.</span>
                  </div>
                </div>

                {/* Right: Cart & Tender Drawer */}
                <div className="md:col-span-5 bg-white rounded-2xl border border-amber-200/90 p-4 shadow-xs flex flex-col justify-between">
                  <div>
                    ...
                    <div className="max-h-36 overflow-y-auto space-y-1.5 pr-1 text-xs">
                      {posCart.map(item => (
                        <div key={item.id} className="flex items-center justify-between bg-stone-50 p-2 rounded-xl border border-stone-100">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button onClick={() => handleRemoveItem(item.id)} className="text-stone-400 hover:text-rose-600 cursor-pointer">
                  ...
                  <div className="pt-3 border-t border-amber-200/80 space-y-2.5">
                    ...
                    <div className="grid grid-cols-3 gap-1.5 text-[10px] font-bold">
                      <button
                        ...
                        className={`py-1.5 rounded-xl flex items-center justify-center gap-1 cursor-pointer border ${
                    ...
                    {printedBill ? (
                      <div className="bg-amber-100 border border-amber-300 p-2.5 rounded-xl text-center text-xs font-black text-amber-900 flex items-center justify-center gap-1.5 animate-fadeIn">
                        ...
                        <span>Invoice #INV-00482 Printed • Loyalty +15 Pts!</span>
                      </div>
                    ) : (
                      <button
          ...
          {selectedRole === 'TENANT_ADMIN' && (
            <div className="space-y-4">
              <div className="flex flex-wrap items-center justify-between gap-3 bg-white p-4 rounded-2xl border border-amber-200/80 shadow-xs">
                <div>
                  <h4 className="font-extrabold text-stone-900 text-sm">MegaMart Retail Group HQ — Executive Console</h4>
                  <p className="text-[11px] text-stone-500">Live aggregated metrics across 6 operational supermarket outlets in Mumbai & Pune</p>
                </div>
                <div className="flex items-center gap-2">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div className="bg-white p-3.5 rounded-2xl border border-amber-200/90">
                  <span className="text-[10px] uppercase font-bold text-stone-500">Total Group Gross (Today)</span>
                  <div className="text-xl font-black text-amber-950 font-mono mt-1">₹4,85,920</div>
                  <span className="text-[10px] text-emerald-600 font-bold flex items-center gap-0.5 mt-1">
                    <TrendingUp className="w-3 h-3" /> +18.4% vs last week
                  </span>
                </div>
                <div className="bg-white p-3.5 rounded-2xl border border-amber-200/90">
                  <span className="text-[10px] uppercase font-bold text-stone-500">Net Profit Margin</span>
                  <div className="text-xl font-black text-stone-900 font-mono mt-1">31.8%</div>
                  <span className="text-[10px] text-emerald-600 font-bold mt-1 block">Healthy retail benchmark</span>
                </div>
                <div className="bg-white p-3.5 rounded-2xl border border-amber-200/90">
                  <span className="text-[10px] uppercase font-bold text-stone-500">Bills Generated</span>
                  <div className="text-xl font-black text-stone-900 font-mono mt-1">1,248</div>
                  <span className="text-[10px] text-stone-500 mt-1 block">Avg. Basket ₹389.30</span>
                </div>
                <div className="bg-white p-3.5 rounded-2xl border border-amber-200/90">
                  <span className="text-[10px] uppercase font-bold text-stone-500">Active Tenant Plan</span>
                  <div className="text-base font-black text-amber-800 mt-1 truncate">Enterprise Hyper-Scale</div>
                  <span className="text-[10px] text-stone-500 mt-1 block">50 Stores Capacity</span>
                </div>
              </div>

              <div className="bg-white p-4 rounded-2xl border border-amber-200/90 space-y-3">
                  ...
                  {[
                    ...
                    <div key={idx} className="space-y-1">
                      <div className="flex justify-between text-xs font-medium text-stone-700">
                        <span>{b.branch}</span>
                        <span className="font-mono font-bold text-stone-900">{b.sales} ({b.pct})</span>
                      </div>
          ...
          {selectedRole === 'INVENTORY_CLERK' && (
            <div className="space-y-4">
              <div className="bg-white p-4 rounded-2xl border border-amber-200/80 shadow-xs flex justify-between items-center">
                <div>
                  <h4 className="font-extrabold text-stone-900 text-sm">FEFO Logistics & Smart Replenishment Audit</h4>
                  <p className="text-[11px] text-stone-500">Automated First-Expiry First-Out inventory protection engine</p>
                </div>
                ...
                <table className="w-full text-left text-xs">
                  ...
                  <tbody className="divide-y divide-stone-100">
                    <tr className="bg-rose-50/50">
                      ...
                      <td className="p-3 text-right"><button className="gold-button-primary text-[10px] py-1 px-2.5 rounded-lg">Push to POS</button></td>
                    </tr>
                    <tr>
                      ...
                      <td className="p-3 text-right"><button className="gold-btn-secondary text-[10px] py-1 px-2.5 rounded-lg">Transfer</button></td>
                    </tr>
                    <tr>
          ...
          {selectedRole === 'STORE_MANAGER' && (
            <div className="space-y-4">
              <div className="bg-white p-4 rounded-2xl border border-amber-200/80 shadow-xs flex justify-between items-center">
                <div>
                  <h4 className="font-extrabold text-stone-900 text-sm">Shift Register Balancing & Staff Attendance</h4>
                  <p className="text-[11px] text-stone-500">Zero-variance cash drawer reconciliation at shift handover</p>
                </div>
                <span className="bg-emerald-100 text-emerald-800 font-black text-[10px] px-3 py-1 rounded-full">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="bg-white p-4 rounded-2xl border border-stone-200">
                  ...
                  <div className="text-lg font-black text-stone-900 font-mono mt-1">₹5,000.00</div>
                ...
                <div className="bg-white p-4 rounded-2xl border border-stone-200">
                  ...
                  <div className="text-lg font-black text-amber-950 font-mono mt-1">₹43,250.00</div>
                  <span className="text-[10px] text-emerald-600 font-bold">142 Cash Invoices</span>
                </div>
                <div className="bg-white p-4 rounded-2xl border border-amber-300 bg-amber-50/50">
                  <span className="text-[10px] text-amber-900 uppercase font-bold">Final Drawer Total</span>
                  <div className="text-lg font-black text-amber-950 font-mono mt-1">₹48,250.00</div>
                  <span className="text-[10px] text-emerald-700 font-bold">Verified by Manager PIN</span>
                </div>
              </div>

              <div className="bg-white p-4 rounded-2xl border border-amber-200/90 text-xs flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <Clock className="w-5 h-5 text-amber-700" />
                  <div>
                    <span className="font-bold text-stone-900 block">Peak Footfall Rush Alert (06:00 PM – 09:00 PM)</span>
                    <span className="text-[11px] text-stone-500">System recommends keeping all 4 express cashier lanes active</span>
                  </div>
                </div>
                <button className="gold-button-primary text-xs py-2 px-3 rounded-xl cursor-pointer">
                  Assign Staff PIN
                </button>
              </div>
            </div>
          ...
          {selectedRole === 'ACCOUNTANT' && (
            <div className="space-y-4">
              <div className="bg-white p-4 rounded-2xl border border-amber-200/80 shadow-xs flex justify-between items-center">
                <div>
                  <h4 className="font-extrabold text-stone-900 text-sm">GSTR-1 & Financial Ledger Reconciliation</h4>
                  <p className="text-[11px] text-stone-500">Automated Indian GST tax classification by HSN / SAC Code</p>
                </div>
                <button className="gold-button-primary text-[10px] py-1.5 px-3 rounded-xl cursor-pointer flex items-center gap-1">
                  <Printer className="w-3.5 h-3.5" />
              ...
              <div className="bg-white p-4 rounded-2xl border border-amber-200/90 text-xs space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2 pt-1 font-mono">
                  <div className="p-2 bg-stone-50 rounded-xl border border-stone-100 text-center">
                    ...
                    <strong className="text-emerald-700">₹2,82,400 (58%)</strong>
                  </div>
                  <div className="p-2 bg-stone-50 rounded-xl border border-stone-100 text-center">
                    <span className="text-[10px] text-stone-500 block">Cards (Stripe POS)</span>
                    <strong className="text-blue-700">₹1,16,600 (24%)</strong>
                  </div>
                  <div className="p-2 bg-stone-50 rounded-xl border border-stone-100 text-center">
                    <span className="text-[10px] text-stone-500 block">Cash Tender</span>
                    <strong className="text-amber-900">₹86,920 (18%)</strong>
                  </div>
                </div>
              </div>
            </div>
          ...
          {selectedRole === 'CUSTOMER_SERVICE' && (
            <div className="space-y-4">
              <div className="bg-white p-4 rounded-2xl border border-amber-200/80 shadow-xs flex justify-between items-center">
                <div>
                  <h4 className="font-extrabold text-stone-900 text-sm">Customer Returns & Instant Bill Reprint</h4>
                  <p className="text-[11px] text-stone-500">Barcode validation for seamless returns without retail friction</p>
                </div>
                <span className="bg-amber-100 text-amber-900 font-bold text-[10px] px-2.5 py-1 rounded-full">
                  Customer Desk
                </span>
              ...
              <div className="bg-white p-4 rounded-2xl border border-amber-200/90 space-y-3">
                <div className="flex gap-2">
                  ...
                  />
                  <button className="gold-button-primary text-xs px-4 rounded-xl cursor-pointer flex items-center gap-1">
                ...
                <div className="p-3 bg-stone-50 rounded-xl border border-stone-200 text-xs space-y-2">
                  <div className="flex justify-between font-bold text-stone-900">
                    <span>Invoice #INV-000003 • Customer: Siddheshwar More</span>
                    <span className="text-emerald-700">Bill Total: ₹783.00</span>
                  </div>
                  <p className="text-[11px] text-stone-500">Date: 14 Sep 2026, 12:45 PM • Cashier Register #02 • Payment: Razorpay UPI</p>
                  <div className="pt-2 border-t border-stone-200 flex items-center justify-between">
                    <span className="text-[11px] text-stone-600">Eligible for 7-day return window</span>
                    <button className="gold-button-primary text-[10px] py-1 px-3 rounded-lg cursor-pointer">
          ...
          {selectedRole === 'SUPER_ADMIN' && (
            <div className="space-y-4">
              <div className="bg-white p-4 rounded-2xl border border-amber-200/80 shadow-xs flex justify-between items-center">
                <div>
                  <h4 className="font-extrabold text-stone-900 text-sm">Multi-Tenant SaaS Platform Operations</h4>
                  <p className="text-[11px] text-stone-500">Enterprise health telemetry across all subscribed retail chains</p>
                </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-white p-3.5 rounded-2xl border border-amber-200 text-xs">
                  ...
                  <div className="text-lg font-black text-amber-950 font-mono mt-1">₹1.48 Crore</div>
                ...
                <div className="bg-white p-3.5 rounded-2xl border border-amber-200 text-xs">
                  ...
                  <div className="text-lg font-black text-stone-900 font-mono mt-1">42 Enterprise Chains</div>
                ...
                <div className="bg-white p-3.5 rounded-2xl border border-amber-200 text-xs">
                  ...
                  <div className="text-lg font-black text-emerald-700 font-mono mt-1">100% Isolated</div>
        ...
        <div className="bg-amber-950 text-amber-100 px-4 md:px-6 py-4 border-t border-amber-800/80 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="space-y-1 text-center md:text-left">
            <div className="flex items-center justify-center md:justify-start gap-2">
              <span className="bg-amber-400 text-amber-950 font-black text-[10px] px-2 py-0.5 rounded-full uppercase">
                {activeRoleData.stat}
              </span>
              <span className="text-xs font-bold text-white">{activeRoleData.tagline}</span>
            </div>
            <p className="text-[11px] text-amber-300/80">{activeRoleData.roi}</p>
          </div>

          <div className="flex items-center gap-3 shrink-0">
            <button
              onClick={() => handleTestDriveRole(selectedRole)}
              className="gold-button-primary text-xs px-5 py-2.5 rounded-xl cursor-pointer flex items-center gap-1.5 shadow-lg shadow-amber-500/20"
            >
